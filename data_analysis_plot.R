...
names(colors) <- c("x", "xa")
eps0_x_label <- as.character(round(dat[dat$model == "x", ]$eps_zero[1] * 100))
eps0_xa_label <- as.character(round(dat[dat$model == "xa", ]$eps_zero[1] * 100))
unconfound_label <- as.character(round(dat$lb[dat$epsilon == 0][1] * 100, 1))
...
ggplot() + 
  geom_ribbon(data = dat[dat$model == "xa", ], 
              ...
              alpha = 0.25) +
  geom_ribbon(data = dat[dat$model == "xa", ], 
              aes(x=epsilon, ymin = ci_lo_ptw_im04, ymax = ci_hi_ptw_im04),
